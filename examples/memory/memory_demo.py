...
logging.basicConfig(
    ...
    format='%(levelname)s: %(message)s' if LOG_LEVEL == 'WARNING' else '%(asctime)s [%(levelname)s] %(name)s: %(message)s'
)
...
async def step_create_profile_schema(create_profile_schema: CreateProfileSchema) -> str:
    """创建用户画像 Schema"""
    ...
    print_success(f"✓ 已创建用户画像 Schema")
...
def example_messages() -> List[Message]:
    return [
        Message(role="user", content="每天上午9点提醒我喝水，下午3点复习笔记。"),
        Message(role="assistant", content="好的，我已经记录下来。"),
        Message(role="user",
                content="还有明天记得提醒我给诺成老师买个生日礼物，诺成老师今年30岁了，比我大三岁。我们的爱好相同，经常一起踢球，所以我打算给诺成老师买一个精美的足球"),
        Message(role="assistant", content="好的，我明天会提醒你"),
    ]


async def step_add_memory(add_memory: AddMemory, end_user_id: str, profile_schema_id: str) -> List[str]:
    """添加对话记忆到记忆服务"""
    ...
    print_info(f"  · 时间戳：{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(payload.timestamp))}")
    print_info(f"  · 对话消息数：{len(payload.messages)} 条")
    print_info(f"  · 元数据：位置={payload.meta_data.get('location_name')}, 坐标={payload.meta_data.get('geo_coordinate')}")
    print("")
    ...
    # 调试：打印返回结果类型
    print_info(f"🔍 调试信息：memory_nodes 类型 = {type(add_result.memory_nodes)}")
    ...
    if isinstance(add_result.memory_nodes, list):
        ...
        # 如果是单个对象，包装成列表
        memory_nodes_list = [add_result.memory_nodes] if add_result.memory_nodes else []
    
    node_ids = [n.memory_node_id for n in memory_nodes_list if n.memory_node_id]
...
async def step_list_memory(list_memory: ListMemory, end_user_id: str, page_num: int = 1, page_size: int = 10) -> List[str]:
    """列出用户的所有记忆条目（分页）"""
    print_info("列出该用户当前保存的所有记忆条目（分页查询）。")
    print("")
    
    payload = ListMemoryInput(user_id=end_user_id, page_num=page_num, page_size=page_size)
    ...
    print_info(f"📊 分页信息：第 {result.page_num}/{total_pages} 页，每页 {result.page_size} 条，共 {result.total} 条")
    print("")
...
async def step_search_memory_with_llm(search_memory: SearchMemory, llm: BaseLLM, end_user_id: str) -> Tuple[List[str], str]:
    """检索记忆并使用大模型生成个性化回答"""
    user_query = "今天和明天需要提醒我做什么？"
    
    print_info("我们将用一个自然语言问题来检索相关记忆，然后让大模型基于这些记忆生成个性化回答。")
    print("")
    ...
    context_lines = [f"- {node.content}" for node in search_result.memory_nodes]
    system_prompt = (
    ...
    char_count = 0
    async for chunk in llm.astream(model=model_name, messages=messages, parameters=params):
        if chunk.choices:
...
async def step_get_user_profile(get_user_profile: GetUserProfile, schema_id: str, end_user_id: str) -> None:
    """获取并展示用户画像信息"""
    print_info("🎯 用户画像功能展示")
    print("")
    print_info("💡 说明：记忆服务会自动从对话中提取用户信息，填充到画像字段中。")
    ...
    if result.profile.attributes:
        print("┌" + "─" * 68 + "┐")
        print(f"│ 👤 用户画像（共 {len(result.profile.attributes)} 个字段）{' ' * (49 - len(str(len(result.profile.attributes))))}│")
        print("├" + "─" * 68 + "┤")
        ...
        for idx, attr in enumerate(result.profile.attributes, start=1):
            ...
            # 字段名
            print(f"│ [{idx}] {attr.name:<61}│")
            
            # 字段值（可能很长，需要处理）
            if len(str(value_display)) <= 60:
                print(f"│     值: {str(value_display):<58}│")
            else:
                # 长文本分行显示
                value_str = str(value_display)
                print(f"│     值: {value_str[:58]:<58}│")
                for i in range(58, len(value_str), 62):
                    line = value_str[i:i+62]
                    print(f"│        {line:<62}│")
            
            # 字段ID
            id_display = truncate(attr.id, 58)
            print(f"│     ID: {id_display:<58}│")
        ...
        if has_values:
            print_success("💡 提示：上述画像信息是记忆服务自动从对话中提取的！")
            print("")
        else:
            print_info("💡 提示：画像字段暂未填充，随着更多对话的积累，会逐步完善。")
            print("")
    else:
        print_info("(暂无画像字段)")
        print("")
...
async def step_delete_memory(delete_memory: DeleteMemory, end_user_id: str, node_ids: List[str]) -> None:
    """删除指定的记忆条目"""
    ...
    for idx, node_id in enumerate(node_ids, start=1):
        ...
        print_success(f"  ✓ [{idx}/{len(node_ids)}] 已删除：{truncate(node_id, 50)}")
        print_info(f"      请求ID：{result.request_id}")
...
async def main() -> None:
    ...
    try:
        ...
        except (MemoryAPIError, MemoryAuthenticationError, MemoryValidationError) as e:
        ...
        try:
            node_ids = await step_add_memory(add_memory, end_user_id, schema_id)
        except (MemoryAPIError, MemoryAuthenticationError, MemoryValidationError) as e:
            print_error("❌ 添加记忆失败：")
            print_error(f"    {format_api_error(e)}")
        ...
        try:
            existing_ids = await step_list_memory(list_memory, end_user_id)
        except (MemoryAPIError, MemoryAuthenticationError, MemoryValidationError) as e:
            print_error("❌ 列出记忆失败：")
            print_error(f"    {format_api_error(e)}")
            # 非关键步骤，可以继续
        ...
        try:
            _hits, _query = await step_search_memory_with_llm(search_memory, llm, end_user_id)
        except (MemoryAPIError, MemoryAuthenticationError, MemoryValidationError) as e:
            print_error("❌ 搜索记忆失败：")
            print_error(f"    {format_api_error(e)}")
            # 非关键步骤，可以继续
        ...
        try:
            await step_get_user_profile(get_user_profile, schema_id, end_user_id)
        except (MemoryAPIError, MemoryAuthenticationError, MemoryValidationError, MemoryNotFoundError) as e:
